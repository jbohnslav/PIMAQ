...
class Device:
    ...
    def update_settings(self):
        ...
        # set this to 2 for slave mode, 1 for master! 0 for default
        ir_sensors.set_option(rs.option.inter_cam_sync_mode, 2)
    ...
    def write_frames(self,left,right, time_acq,cputime):
        ...
        if ret1 and ret2:
            ...
            append_to_hdf5(self.fileobj,'sestime', time_acq)
...
def run_loop(device):
    ...
    try:
        while True:
            # print('acquiring')
            frames = device.pipeline.wait_for_frames(1000*15)
            ...
            time_acq = time.perf_counter() - device.start_t
            cputime = time.time()
            if device.save:
...
def main():
    ...
    start_t = time.perf_counter()
    device = Device('830112071475', config, start_t, save=args.save, savedir=datadir, 
        experiment=experiment,
        name='right_post', preview=args.preview)
